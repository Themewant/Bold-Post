...
$style_handle = 'bldpst-post-grid-style';
$unique_id    = 'bldpst-' . wp_rand( 100, 99999 );
$selector     = '.' . $unique_id;

$full_responsive_css = Bold_Post_Helper::generate_responsive_css($selector . ' .bldpst-post-grid.style-' . $style, $responsive_data);
$full_responsive_css .= Bold_Post_Helper::generate_responsive_css($selector . ' .bldpst-post-grid.style-' . $style . ' .bldpst-grid-item', $item_responsive);
$full_responsive_css .= Bold_Post_Helper::generate_responsive_css($selector . ' .bldpst-post-grid.style-' . $style . ' .bldpst-grid-item .bldpst-blog-title', $title_responsive);
$full_responsive_css .= Bold_Post_Helper::generate_responsive_css($selector . ' .bldpst-post-grid.style-' . $style . ' .bldpst-grid-item .bldpst-blog-excerpt', $excerpt_responsive);

wp_enqueue_style( $style_handle );
Bold_Post_Helper::add_custom_style( $style_handle, $selector, $full_responsive_css, [
    '.bldpst-post-grid .bldpst-grid-item:hover'    => Bold_Post_Helper::get_inline_styles($item_hover),
    '.bldpst-post-grid .bldpst-grid-item .bldpst-overlay-all'        => Bold_Post_Helper::get_inline_styles($overlay_styles),
    '.bldpst-post-grid .bldpst-grid-item .bldpst-blog-title a:hover' => Bold_Post_Helper::get_inline_styles($title_hover),
    '.bldpst-post-grid .bldpst-grid-item .bldpst-blog-excerpt a:hover'=> Bold_Post_Helper::get_inline_styles($excerpt_hover),
    '.bldpst-post-grid .bldpst-grid-item .bldpst-read-more-link'     => Bold_Post_Helper::get_inline_styles($button_styles),
    '.bldpst-post-grid .bldpst-grid-item .bldpst-read-more-link:hover'=> Bold_Post_Helper::get_inline_styles($button_hover),
    '.bldpst-post-grid .bldpst-grid-item .bldpst-blog-date-top'      => Bold_Post_Helper::get_inline_styles($td_styles),
    '.bldpst-post-grid .bldpst-grid-item .bldpst-blog-metas'         => Bold_Post_Helper::get_inline_styles($meta_styles),
    '.bldpst-post-grid .bldpst-pagination a, .bldpst-pagination span' => Bold_Post_Helper::get_inline_styles($pag_styles),
...
$block_wrap_attr = get_block_wrapper_attributes( array( 'class' => 'bldpst-post-grid-block-wrap ' . $unique_id ) );

if ( $query->have_posts() ) :
?>
    <div <?php echo wp_kses_post($block_wrap_attr); ?>>
        <div class="bldpst-post-grid style-<?php echo esc_attr($style); ?>">
            <?php
            while ( $query->have_posts() ) : $query->the_post();
                ...
                $style_file = BLDPST_PL_PATH . 'public/template-parts/blog-grid/style-' . $style . '.php';
